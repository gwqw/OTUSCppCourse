...
BOOST_AUTO_TEST_CASE(test_ip_filter_equal) {
    {
        ...
        auto res_rng = ip_pool | ranges::view::filter([value=1](const auto& ip){ return ip[0] == value;});
    ...
    }
    {
        ...
        auto res_rng = ip_pool | ranges::view::filter([](const auto& ip){ return ip[0] == 46;})
                               | ranges::view::filter([](const auto& ip){ return ip[1] == 70;});
        IPPool res{res_rng.begin(),res_rng.end()};
...
BOOST_AUTO_TEST_CASE(test_ip_filter_any) {
    {
        ...
        auto res_rng = ip_pool | ranges::view::filter([value=46](const IPAddress& ip) {
