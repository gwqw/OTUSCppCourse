...
BulkCmdManager::BulkCmdManager(std::size_t bulk_max_size)
    : bulk_capacity_(bulk_max_size),
      ...
      cout_tread_pool_(1, "log"),
      file_tread_pool_(THREADS_NUM, "file")
{
}
...
void BulkCmdManager::add_cmd(Command cmd) {
    if (cmd.cmd_type != CommandType::Terminator) ++main_metric_.line_num;
...
void GeneralStateHandler::handle_cmd(BulkCmdManager *m, Command cmd) {
    switch (cmd.cmd_type) {
        case CommandType::Base:
            ...
            ++m->main_metric_.command_num;
            if (m->main_metric_.block_num == 0) m->main_metric_.block_num = 1;
            break;
        case CommandType::StartCustomBulk:
            m->flush_data();
            m->bulk_handler_ = make_unique<CustomStateHandler>();
            ++m->nesting_counter_;
            ++m->main_metric_.block_num;
            break;
    ...
    switch (cmd.cmd_type) {
        ...
        case CommandType::StopCustomBulk:
            ...
            if (m->nesting_counter_ == 0) {
                m->main_metric_.command_num += m->cur_bulk_.data_.size();
                m->flush_data();
...
void BulkCmdManager::print_metric(std::ostream &out) const {
    out << main_metric_;
}
